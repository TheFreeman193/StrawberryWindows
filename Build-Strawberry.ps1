...
begin {
    ...
    [string]$DependsPath = "${DependsPathRoot}_${BuildType}_$BuildArch"
...
"@
    ...
    $env:CL = '-MP'
    $env:PATH = "$DependsPath\bin;$env:PATH"
    ...
    if ([string]::IsNullOrWhiteSpace($env:VSCMD_VER) -or $env:VSCMD_VER -notmatch '^17\.\d+\.\d+$' -or $env:VSCMD_ARG_TGT_ARCH -ne $BuildArch) {
        ...
        if ([string]::IsNullOrWhiteSpace($VSToolsPath) -or -not (Test-Path $VSToolsPath -PathType Container) -or -not (Test-Path $VSPSPath -PathType Leaf)) {
            $Err = [ErrorRecord]::new([DirectoryNotFoundException]::new('Visual Studio 2022 not found.'), 'VisualStudioNotFound', 'ObjectNotFound', 'C:\Program Files\Microsoft Visual Studio\2022')
        ...
        if ($env:VSCMD_VER -notmatch '^17\.\d+\.\d+$' -or $env:VSCMD_ARG_TGT_ARCH -ne $BuildArch -or $null -eq (Get-Command -CommandType Application 'nmake.exe')) {
            $Err = [ErrorRecord]::new([InvalidPowerShellStateException]::new('Unable to properly configure the VS 2002 Dev Shell'), 'VisualStudioConfigError', 'ResourceUnavailable', 'C:\Program Files\Microsoft Visual Studio\2022')
            $PSCmdlet.WriteError($Err)
            return
        }
    ...
    foreach ($CriticalPath in $BuildPath, $DependsPath, "$DependsPath\bin", "$DependsPath\lib", "$DependsPath\include", $TempPath) {
        Write-Host -fo White ('{0,-70}' -f "Checking source/build path '$CriticalPath'...  ") -NoNewline
    ...
    foreach ($BinPath in $ToolsList.Keys) {
        ...
        Write-Host -fo White ('{0,-70}' -f "Checking '$Command'...  ") -NoNewline
        if ($null -eq (Get-Command -CommandType Application $Command -ErrorAction Ignore)) {
            ...
            if (-not $Success) {
                ...
                $Err = [ErrorRecord]::new([CommandNotFoundException]::new("Build tool '$Command' not found! Please install $($ToolsList[$BinPath])"), 'CommandNotFound', 'ObjectNotFound', $BinPath)
                $PSCmdlet.WriteError($Err)
                return
            }
...
process {
    ...
    function BuildYasm {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            ...
            tar.exe -xf "$DownloadPath\yasm-$YASM_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Patch build configuration...'
        Get-Content "$DownloadPath\yasm-cmake.patch" -ErrorAction Stop | patch.exe -p1 -N -d $LocalBuildPath
        ...
        cmake.exe --log-level="DEBUG" -S $LocalBuildPath -B "$LocalBuildPath\build" -G 'NMake Makefiles' -DCMAKE_BUILD_TYPE="$BuildTypeCMake" -DCMAKE_INSTALL_PREFIX="$DependsPathForward" -DPKG_CONFIG_EXECUTABLE="$DependsPathForward/bin/pkgconf.exe" -DCMAKE_POLICY_VERSION_MINIMUM="3.5"
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    CMake build...'
        cmake.exe --build "$LocalBuildPath\build"
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    CMake install...'
        cmake.exe --install "$LocalBuildPath\build"
    }
    ...
    function BuildPkgConf {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            tar.exe -xf "$DownloadPath\pkgconf-$PKGCONF_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        if (-not (Test-Path "$LocalBuildPath\build\build.ninja")) {
            Write-Host -fo Cyan '    Meson configure...'
            meson.exe setup --buildtype="$BuildTypeMeson" --default-library=shared --prefix="$DependsPath" --wrap-mode=nodownload -Dtests=disabled "$LocalBuildPath\build" $LocalBuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Ninja build...'
        ninja.exe -C "$LocalBuildPath\build"
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    Ninja install...'
        ninja.exe -C "$LocalBuildPath\build" install
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    Copy pkgconf -> pkg-config...'
        Copy-Item "$DependsPath\bin\pkgconf.exe" "$DependsPath\bin\pkg-config.exe" -Force
    }
    ...
    function BuildMimAlloc {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            tar.exe -xf "$DownloadPath\mimalloc-$MIMALLOC_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
        ...
        Write-Host -fo Cyan '    CMake configure...'
        cmake.exe --log-level="DEBUG" -S $LocalBuildPath -B "$LocalBuildPath\build" -G 'NMake Makefiles' -DCMAKE_BUILD_TYPE="$BuildTypeCMake" -DCMAKE_INSTALL_PREFIX="$DependsPathForward" -DPKG_CONFIG_EXECUTABLE="$DependsPathForward/bin/pkgconf.exe" -DBUILD_SHARED_LIBS=ON -DBUILD_STATIC_LIBS=OFF -DMI_BUILD_SHARED=ON -DMI_BUILD_STATIC=OFF -DMI_BUILD_TESTS=OFF -DMI_CHECK_FULL=OFF -DMI_DEBUG_FULL=OFF -DMI_DEBUG_TSAN=OFF -DMI_DEBUG_UBSAN=OFF -DMI_OVERRIDE=ON -DMI_USE_CXX=ON -DMI_WIN_REDIRECT=ON
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    CMake build...'
        cmake.exe --build "$LocalBuildPath\build"
        if ($LASTEXITCODE -ne 0) { return }
    ...
    function BuildGetOpt {
        ...
        Write-Host -fo Cyan '    Patch build configuration...'
        Get-Content "$DownloadPath\getopt-win-cmake.patch" | patch.exe -p1 -N -d $LocalBuildPath
        if ($Error.Count -gt 0 -or $LASTEXITCODE -gt 1) { return }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
            $null = New-Item "$LocalBuildPath\build" -ItemType Directory -Force
        ...
        Write-Host -fo Cyan '    CMake configure...'
        cmake.exe --log-level="DEBUG" -S $LocalBuildPath -B "$LocalBuildPath\build" -G 'NMake Makefiles' -DCMAKE_BUILD_TYPE="$BuildTypeCMake" -DCMAKE_INSTALL_PREFIX="$DependsPathForward" -DPKG_CONFIG_EXECUTABLE="$DependsPathForward/bin/pkgconf.exe"
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    CMake build...'
        cmake.exe --build "$LocalBuildPath\build"
        if ($LASTEXITCODE -ne 0) { return }
    ...
    function BuildZlib {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            tar.exe -xf "$DownloadPath\zlib-$ZLIB_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
        ...
        Write-Host -fo Cyan '    CMake configure...'
        cmake.exe --log-level="DEBUG" -S $LocalBuildPath -B "$LocalBuildPath\build" -G 'NMake Makefiles' -DCMAKE_BUILD_TYPE="$BuildTypeCMake" -DCMAKE_INSTALL_PREFIX="$DependsPathForward" -DPKG_CONFIG_EXECUTABLE="$DependsPathForward/bin/pkgconf.exe" -DBUILD_SHARED_LIBS=ON -DBUILD_STATIC_LIBS=OFF
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    CMake build...'
        cmake.exe --build "$LocalBuildPath\build"
        if ($LASTEXITCODE -ne 0) { return }
    ...
    function BuildOpenSSL {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            tar.exe -xf "$DownloadPath\openssl-$OPENSSL_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Push-Location $LocalBuildPath
        Write-Host -fo Cyan '    Perl configure...'
        $ZLibInfix = if ($BuildArch -eq 'arm64') { '' } else { 'zlib' }
        perl.exe Configure $PerlArch shared $ZLibInfix no-capieng no-tests --prefix="$DependsPath" --libdir="lib" --openssldir="$DependsPath\ssl" --$BuildType --with-zlib-include="$DependsPath\include" --with-zlib-lib="$DependsPath\lib\zlib.lib"
        if ($LASTEXITCODE -ne 0) { Pop-Location; return }
        Write-Host -fo Cyan '    NMake build...'
        nmake.exe /f "$LocalBuildPath\makefile" MAKEDIR="$LocalBuildPath"
        if ($LASTEXITCODE -ne 0) { Pop-Location; return }
        Write-Host -fo Cyan '    NMake install...'
        nmake.exe /f "$LocalBuildPath\makefile" MAKEDIR="$LocalBuildPath" install_sw
        if ($LASTEXITCODE -ne 0) { Pop-Location; return }
        Pop-Location
        Write-Host -fo Cyan '    Copy libraries...'
...
"@
    ...
    function BuildGnutls {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            ...
            7z.exe x -aoa "$DownloadPath\libgnutls_${GNUTLS_VERSION}_msvc17.zip" -o"$LocalBuildPath"
            if ($LASTEXITCODE -ne 0) { return }
        }
...
"@
    ...
    function BuildLibpng {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            tar.exe -xf "$DownloadPath\libpng-$LIBPNG_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Patch build configuration...'
        # Get-Content "$DownloadPath\libpng-pkgconf.patch" | patch.exe -p1 -N -d $LocalBuildPath
        (Get-Content "$LocalBuildPath\CMakeLists.txt" -Raw) -replace '(?ms)^(\s*)(if\(NOT (?:CMAKE_HOST_)?WIN32 OR CYGWIN OR MINGW\))(.+?)^\1(endif\(\))', '$1# $2$3$1# $4' -replace
    ...
    function BuildLibjpeg {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            tar.exe -xf "$DownloadPath\libjpeg-turbo-$LIBJPEG_TURBO_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
        ...
        Write-Host -fo Cyan '    CMake configure...'
        cmake.exe --log-level="DEBUG" -S $LocalBuildPath -B "$LocalBuildPath\build" -G 'NMake Makefiles' -DCMAKE_BUILD_TYPE="$BuildTypeCMake" -DCMAKE_INSTALL_PREFIX="$DependsPathForward" -DPKG_CONFIG_EXECUTABLE="$DependsPathForward/bin/pkgconf.exe" -DBUILD_SHARED_LIBS=ON -DENABLE_SHARED=ON -DCMAKE_POLICY_VERSION_MINIMUM="3.5" -DWITH_SIMD="$SIMD"
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    CMake build...'
        cmake.exe --build "$LocalBuildPath\build"
        if ($LASTEXITCODE -ne 0) { return }
    ...
    function BuildPcre2 {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            tar.exe -xf "$DownloadPath\pcre2-$PCRE2_VERSION.tar.bz2" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
        ...
        Write-Host -fo Cyan '    CMake configure...'
        cmake.exe --log-level="DEBUG" -S $LocalBuildPath -B "$LocalBuildPath\build" -G 'NMake Makefiles' -DCMAKE_BUILD_TYPE="$BuildTypeCMake" -DCMAKE_INSTALL_PREFIX="$DependsPathForward" -DPKG_CONFIG_EXECUTABLE="$DependsPathForward/bin/pkgconf.exe" -DBUILD_SHARED_LIBS=ON -DBUILD_STATIC_LIBS=OFF -DPCRE2_BUILD_PCRE2_16=ON -DPCRE2_BUILD_PCRE2_32=ON -DPCRE2_BUILD_PCRE2_8=ON -DPCRE2_BUILD_TESTS=OFF -DPCRE2_SUPPORT_UNICODE=ON
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    CMake build...'
        cmake.exe --build "$LocalBuildPath\build"
        if ($LASTEXITCODE -ne 0) { return }
    ...
    function BuildBzip2 {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            tar.exe -xf "$DownloadPath\bzip2-$BZIP2_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Patch build configuration...'
        Get-Content "$DownloadPath\bzip2-cmake.patch" | patch.exe -p1 -N -d $LocalBuildPath
        if ($Error.Count -gt 0 -or $LASTEXITCODE -gt 1) { return }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
            $null = New-Item "$LocalBuildPath\build" -ItemType Directory -Force
    ...
    function BuildXz {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            tar.exe -xf "$DownloadPath\xz-$XZ_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
        ...
        Write-Host -fo Cyan '    CMake configure...'
        cmake.exe --log-level="DEBUG" -S $LocalBuildPath -B "$LocalBuildPath\build" -G 'NMake Makefiles' -DCMAKE_BUILD_TYPE="$BuildTypeCMake" -DCMAKE_INSTALL_PREFIX="$DependsPathForward" -DPKG_CONFIG_EXECUTABLE="$DependsPathForward/bin/pkgconf.exe" -DBUILD_SHARED_LIBS=ON -DBUILD_STATIC_LIBS=OFF -DBUILD_TESTING=OFF -DXZ_NLS=OFF
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    CMake build...'
        cmake.exe --build "$LocalBuildPath\build"
        if ($LASTEXITCODE -ne 0) { return }
    ...
    function BuildBrotli {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            tar.exe -xf "$DownloadPath\brotli-$BROTLI_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
        ...
        Write-Host -fo Cyan '    CMake configure...'
        cmake.exe --log-level="DEBUG" -S $LocalBuildPath -B "$LocalBuildPath\build" -G 'NMake Makefiles' -DCMAKE_BUILD_TYPE="$BuildTypeCMake" -DCMAKE_INSTALL_PREFIX="$DependsPathForward" -DPKG_CONFIG_EXECUTABLE="$DependsPathForward/bin/pkgconf.exe" -DBUILD_TESTING=OFF
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    CMake build...'
        cmake.exe --build "$LocalBuildPath\build"
        if ($LASTEXITCODE -ne 0) { return }
    ...
    function BuildLibiconv {
        ...
        msbuild.exe "$LocalBuildPath\libiconv.sln" -p:Configuration="$BuildType" -p:Platform="$BuildArch" -p:SkipUWP=true
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    Copy libraries...'
    ...
    function BuildIcu4c {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            7z.exe x "$DownloadPath\icu4c-$ICU4C_VERSION_UNDERSCORE-src.zip" -o"$BuildPath"
            if ($LASTEXITCODE -ne 0) { return }
        }
        ...
        Write-Host -fo Cyan '    MSBuild build...'
        msbuild.exe "$LocalBuildPath\source\allinone\allinone.sln" -p:Configuration="$BuildType" -p:Platform="$BuildArch32Alt" -p:SkipUWP=true
        if ($LASTEXITCODE -ne 0) { return }
...
"@
    ...
    function BuildPixman {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            tar.exe -xf "$DownloadPath\pixman-$PIXMAN_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        if (-not (Test-Path "$LocalBuildPath\build\build.ninja")) {
            Write-Host -fo Cyan '    Meson configure...'
            meson.exe setup --buildtype="$BuildTypeMeson" --default-library=shared --prefix="$DependsPath" --pkg-config-path="$DependsPath\lib\pkgconfig" --wrap-mode=nodownload -Dgtk=disabled -Dlibpng=enabled "$LocalBuildPath\build" $LocalBuildPath | Out-Host
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Ninja build...'
        ninja.exe -C "$LocalBuildPath\build"
    ...
    function BuildExpat {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            tar.exe -xf "$DownloadPath\expat-$EXPAT_VERSION.tar.bz2" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
        ...
        Write-Host -fo Cyan '    CMake configure...'
        cmake.exe --log-level="DEBUG" -S $LocalBuildPath -B "$LocalBuildPath\build" -G 'NMake Makefiles' -DCMAKE_BUILD_TYPE="$BuildTypeCMake" -DCMAKE_INSTALL_PREFIX="$DependsPathForward" -DPKG_CONFIG_EXECUTABLE="$DependsPathForward/bin/pkgconf.exe" -DEXPAT_SHARED_LIBS=ON -DEXPAT_BUILD_DOCS=OFF -DEXPAT_BUILD_EXAMPLES=OFF -DEXPAT_BUILD_FUZZERS=OFF -DEXPAT_BUILD_TESTS=OFF -DEXPAT_BUILD_TOOLS=OFF -DEXPAT_BUILD_PKGCONFIG=ON
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    CMake build...'
        cmake.exe --build "$LocalBuildPath\build"
        if ($LASTEXITCODE -ne 0) { return }
    ...
    function BuildBoost {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            tar.exe -xf "$DownloadPath\boost_$BOOST_VERSION_UNDERSCORE.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        ...
        .\bootstrap.bat msvc
        if ($LASTEXITCODE -ne 0) { Pop-Location; return }
        .\b2.exe -a -q -j $env:NUMBER_OF_PROCESSORS -d2 --ignore-site-config --stagedir="$LocalBuildPath\stage" --build`-dir="$LocalBuildPath\build" --layout="tagged" --prefix="$DependsPath" --exec-prefix="$DependsPath\bin" --libdir="$DependsPath\lib" --includedir="$DependsPath\include" --with-headers architecture=$B2Arch address-model=$BuildAddressSize variant=$BuildType toolset=msvc link=shared runtime-link=shared threadapi=win32 threading=multi install
        Pop-Location
    }
    ...
    function BuildLibxml2 {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            tar.exe -xf "$DownloadPath\libxml2-v$LIBXML2_VERSION.tar.bz2" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
        ...
        Write-Host -fo Cyan '    CMake configure...'
        cmake.exe --log-level="DEBUG" -S $LocalBuildPath -B "$LocalBuildPath\build" -G 'NMake Makefiles' -DCMAKE_BUILD_TYPE="$BuildTypeCMake" -DCMAKE_INSTALL_PREFIX="$DependsPathForward" -DPKG_CONFIG_EXECUTABLE="$DependsPathForward/bin/pkgconf.exe" -DICU_ROOT="$DependsPathForward" -DBUILD_SHARED_LIBS=ON -DLIBXML2_WITH_PYTHON=OFF -DLIBXML2_WITH_ZLIB=ON -DLIBXML2_WITH_LZMA=ON -DLIBXML2_WITH_ICONV=ON -DLIBXML2_WITH_ICU=ON
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    CMake build...'
        cmake.exe --build "$LocalBuildPath\build"
        if ($LASTEXITCODE -ne 0) { return }
    ...
    function BuildNghttp2 {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            tar.exe -xf "$DownloadPath\nghttp2-$NGHTTP2_VERSION.tar.bz2" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
    ...
    function BuildLibffi {
        ...
        if (-not (Test-Path "$LocalBuildPath\build\build.ninja")) {
            Write-Host -fo Cyan '    Meson configure...'
            meson.exe setup --buildtype="$BuildTypeMeson" --default-library=shared --wrap-mode=nodownload --prefix="$DependsPath" --pkg-config-path="$DependsPath\lib\pkgconfig" "$LocalBuildPath\build" $LocalBuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Ninja build...'
        ninja.exe -C "$LocalBuildPath\build"
    ...
    function BuildDlfcn {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            tar.exe -xf "$DownloadPath\dlfcn-win32-$DLFCN_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
        ...
        Write-Host -fo Cyan '    CMake configure...'
        cmake.exe --log-level="DEBUG" -S $LocalBuildPath -B "$LocalBuildPath\build" -G 'NMake Makefiles' -DCMAKE_BUILD_TYPE="$BuildTypeCMake" -DCMAKE_INSTALL_PREFIX="$DependsPathForward" -DPKG_CONFIG_EXECUTABLE="$DependsPathForward/bin/pkgconf.exe" -DBUILD_SHARED_LIBS=ON
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    CMake build...'
        cmake.exe --build "$LocalBuildPath\build"
        if ($LASTEXITCODE -ne 0) { return }
    ...
    function BuildLibpsl {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            tar.exe -xf "$DownloadPath\libpsl-$LIBPSL_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        if (-not (Test-Path "$LocalBuildPath\build\build.ninja")) {
            Write-Host -fo Cyan '    Meson configure...'
            meson.exe setup --buildtype="$BuildTypeMeson" --default-library=shared --wrap-mode=nodownload --prefix="$DependsPath" --pkg-config-path="$DependsPath\lib\pkgconfig" -Dc_args="-I$DependsPathForward/include" -Dc_link_args="-L$DependsPath\lib" "$LocalBuildPath\build" $LocalBuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Ninja build...'
        ninja.exe -C "$LocalBuildPath\build"
    ...
    function BuildOrc {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            7z.exe x -aos "$DownloadPath\orc-$ORC_VERSION.tar.xz" -o"$DownloadPath"
            if ($LASTEXITCODE -ne 0) { return }
            7z.exe x -aoa "$DownloadPath\orc-$ORC_VERSION.tar" -o"$BuildPath"
            if ($LASTEXITCODE -ne 0) { return }
        }
        if (-not (Test-Path "$LocalBuildPath\build\build.ninja")) {
            Write-Host -fo Cyan '    Meson configure...'
    ...
    function BuildLibcurl {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            tar.exe -xf "$DownloadPath\curl-$CURL_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
    ...
    function BuildSqlite {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            tar.exe -xf "$DownloadPath\sqlite-autoconf-$SQLITE3_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
...
"@
    ...
    function BuildGlib {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            7z.exe x -aos "$DownloadPath\glib-$GLIB_VERSION.tar.xz" -o"$DownloadPath"
            if ($LASTEXITCODE -ne 0) { return }
            7z.exe x -aoa "$DownloadPath\glib-$GLIB_VERSION.tar" -o"$BuildPath"
            if ($LASTEXITCODE -ne 0) { return }
        }
        if (-not (Test-Path "$LocalBuildPath\build\build.ninja")) {
            Write-Host -fo Cyan '    Meson configure...'
            meson.exe setup --buildtype="$BuildTypeMeson" --default-library=shared --prefix="$DependsPath" --pkg-config-path="$DependsPath\lib\pkgconfig" --includedir="$DependsPath\include" --libdir="$DependsPath\lib" -Dtests=false -Dc_args="-I$DependsPathForward/include" -Dc_link_args="-L$DependsPath\lib" "$LocalBuildPath\build" $LocalBuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Ninja build...'
        ninja.exe -C "$LocalBuildPath\build"
    ...
    function BuildLibproxy {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            tar.exe -xf "$DownloadPath\libproxy-$LIBPROXY_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Push-Location $LocalBuildPath
        if (-not (Test-Path "$LocalBuildPath\build\build.ninja")) {
            Write-Host -fo Cyan '    Meson configure...'
            meson.exe setup --buildtype="$BuildTypeMeson" --default-library=shared --wrap-mode=nodownload --prefix="$DependsPath" --pkg-config-path="$DependsPath\lib\pkgconfig" --includedir="$DependsPath\include" --libdir="$DependsPath\lib" -Dc_args="-I$DependsPathForward/include" -Drelease="$IsRelease" -Ddocs=false -Dtests=false -Dconfig-xdp=false -Dconfig-kde=false -Dconfig-osx=false -Dconfig-gnome=false -Dpacrunner-duktape=false -Dintrospection=false -DHAVE_CURL=false "$LocalBuildPath\build" $LocalBuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Pop-Location
        Write-Host -fo Cyan '    Ninja build...'
    ...
    function BuildLibsoup {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            7z.exe x -aos "$DownloadPath\libsoup-$LIBSOUP_VERSION.tar.xz" -o"$DownloadPath"
            if ($LASTEXITCODE -ne 0) { return }
            7z.exe x -aoa "$DownloadPath\libsoup-$LIBSOUP_VERSION.tar" -o"$BuildPath"
            if ($LASTEXITCODE -ne 0) { return }
        }
        if (-not (Test-Path "$LocalBuildPath\build\build.ninja")) {
            Write-Host -fo Cyan '    Meson configure...'
            meson.exe setup --buildtype="$BuildTypeMeson" --default-library=shared --prefix="$DependsPath" --pkg-config-path="$DependsPath\lib\pkgconfig" -Dc_args="-I$DependsPathForward/include" --includedir="$DependsPath\include" --libdir="$DependsPath\lib" -Dtests=false -Dvapi=disabled -Dgssapi=disabled -Dintrospection=disabled -Dtests=false -Dsysprof=disabled -Dtls_check=false "$LocalBuildPath\build" $LocalBuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Ninja build...'
        ninja.exe -C "$LocalBuildPath\build"
    ...
    function BuildGlibNetworking {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            7z.exe x -aos "$DownloadPath\glib-networking-$GLIB_NETWORKING_VERSION.tar.xz" -o"$DownloadPath"
            if ($LASTEXITCODE -ne 0) { return }
            7z.exe x -aoa "$DownloadPath\glib-networking-$GLIB_NETWORKING_VERSION.tar" -o"$BuildPath"
            if ($LASTEXITCODE -ne 0) { return }
        }
        if (-not (Test-Path "$LocalBuildPath\build\build.ninja")) {
            Write-Host -fo Cyan '    Meson configure...'
            meson.exe setup --buildtype="$BuildTypeMeson" --default-library=shared --prefix="$DependsPath" --pkg-config-path="$DependsPath\lib\pkgconfig" -Dc_args="-I$DependsPathForward/include" --includedir="$DependsPath\include" --libdir="$DependsPath\lib" -Dgnutls=enabled -Dopenssl=enabled -Dgnome_proxy=disabled -Dlibproxy=disabled "$LocalBuildPath\build" $LocalBuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Ninja build...'
        ninja.exe -C "$LocalBuildPath\build"
    ...
    function BuildFreetype {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            tar.exe -xf "$DownloadPath\freetype-$FREETYPE_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
        ...
        Write-Host -fo Cyan '    CMake configure...'
        cmake.exe --log-level="DEBUG" -S $LocalBuildPath -B "$LocalBuildPath\build" -G 'NMake Makefiles' -DCMAKE_BUILD_TYPE="$BuildTypeCMake" -DCMAKE_INSTALL_PREFIX="$DependsPathForward" -DPKG_CONFIG_EXECUTABLE="$DependsPathForward/bin/pkgconf.exe" -DBUILD_SHARED_LIBS=ON -DFT_DISABLE_HARFBUZZ="$HarfbuzzDisable"
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    CMake build...'
        cmake.exe --build "$LocalBuildPath\build"
        if ($LASTEXITCODE -ne 0) { return }
    ...
    function BuildCairo {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            7z.exe x -aos "$DownloadPath\cairo-$CAIRO_VERSION.tar.xz" -o"$DownloadPath"
            if ($LASTEXITCODE -ne 0) { return }
            7z.exe x -aoa "$DownloadPath\cairo-$CAIRO_VERSION.tar" -o"$BuildPath"
            if ($LASTEXITCODE -ne 0) { return }
        }
        ...
        if (-not (Test-Path "$LocalBuildPath\build\build.ninja")) {
            Write-Host -fo Cyan '    Meson configure...'
            meson.exe setup --buildtype="$BuildTypeMeson" --default-library=shared --prefix="$DependsPath" --pkg-config-path="$DependsPath\lib\pkgconfig" -Dc_args="-I$DependsPathForward/include" --includedir="$DependsPath\include" --libdir="$DependsPath\lib" -Dfontconfig=disabled -Dfreetype=enabled -Dzlib=enabled -Dpng=enabled -Dtests=disabled -Dgtk_doc=false "$LocalBuildPath\build" $LocalBuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Ninja build...'
        ninja.exe -C "$LocalBuildPath\build"
    ...
    function BuildHarfbuzz {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            7z.exe x -aos "$DownloadPath\harfbuzz-$HARFBUZZ_VERSION.tar.xz" -o"$DownloadPath"
            if ($LASTEXITCODE -ne 0) { return }
            7z.exe x -aoa "$DownloadPath\harfbuzz-$HARFBUZZ_VERSION.tar" -o"$BuildPath"
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Patch alloca (Linux) -> _alloca (MSVC)...'
        if (-not (Test-Path "$LocalBuildPath\build\build.ninja")) {
            Write-Host -fo Cyan '    Meson configure...'
            meson.exe setup --buildtype="$BuildTypeMeson" --default-library=shared --prefix="$DependsPath" --pkg-config-path="$DependsPath\lib\pkgconfig" -Dc_args="-I$DependsPathForward/include" -Dcpp_args="-I$DependsPathForward/include" -Dc_link_args="-L$DependsPath\lib" --includedir="$DependsPath\include" --libdir="$DependsPath\lib" -Dcpp_std="c++17" -Dtests=disabled -Ddocs=disabled -Dicu=enabled -Dfreetype=enabled "$LocalBuildPath\build" $LocalBuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Ninja build...'
        ninja.exe -C "$LocalBuildPath\build"
    ...
    function BuildLibogg {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            tar.exe -xf "$DownloadPath\libogg-$LIBOGG_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
        ...
        Write-Host -fo Cyan '    CMake configure...'
        cmake.exe --log-level="DEBUG" -S $LocalBuildPath -B "$LocalBuildPath\build" -G 'NMake Makefiles' -DCMAKE_BUILD_TYPE="$BuildTypeCMake" -DCMAKE_INSTALL_PREFIX="$DependsPathForward" -DPKG_CONFIG_EXECUTABLE="$DependsPathForward/bin/pkgconf.exe" -DBUILD_SHARED_LIBS=ON -DINSTALL_DOCS=OFF -DCMAKE_POLICY_VERSION_MINIMUM="3.5"
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    CMake build...'
        cmake.exe --build "$LocalBuildPath\build"
        if ($LASTEXITCODE -ne 0) { return }
    ...
    function BuildLibvorbis {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            tar.exe -xf "$DownloadPath\libvorbis-$LIBVORBIS_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
    ...
    function BuildFlac {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            7z.exe x -aos "$DownloadPath\flac-$FLAC_VERSION.tar.xz" -o"$DownloadPath"
            if ($LASTEXITCODE -ne 0) { return }
            7z.exe x -aoa "$DownloadPath\flac-$FLAC_VERSION.tar" -o"$BuildPath"
            if ($LASTEXITCODE -ne 0) { return }
        }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
        ...
        Write-Host -fo Cyan '    CMake configure...'
        cmake.exe --log-level="DEBUG" -S $LocalBuildPath -B "$LocalBuildPath\build" -G 'NMake Makefiles' -DCMAKE_BUILD_TYPE="$BuildTypeCMake" -DCMAKE_INSTALL_PREFIX="$DependsPathForward" -DPKG_CONFIG_EXECUTABLE="$DependsPathForward/bin/pkgconf.exe" -DBUILD_SHARED_LIBS=ON -DBUILD_DOCS=OFF -DBUILD_EXAMPLES=OFF -DINSTALL_MANPAGES=OFF -DBUILD_TESTING=OFF -DBUILD_PROGRAMS=OFF
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    CMake build...'
        cmake.exe --build "$LocalBuildPath\build"
        if ($LASTEXITCODE -ne 0) { return }
    ...
    function BuildWavpack {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            tar.exe -xf "$DownloadPath\wavpack-$WAVPACK_VERSION.tar.bz2" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
        ...
        Write-Host -fo Cyan '    Patch cmake root -> lib/cmake...'
        (Get-Content "$LocalBuildPath\CMakeLists.txt") -creplace '(set\(CMAKE_INSTALL_PACKAGEDIR )cmake\)', '$1$${CMAKE_INSTALL_LIBDIR}/cmake/$${PROJECT_NAME})' | Set-Content "$LocalBuildPath\CMakeLists.txt"
        Write-Host -fo Cyan '    CMake configure...'
        ...
        Copy-Item "$DependsPath\lib\wavpackdll.lib" "$DependsPath\lib\wavpack.lib"
        if (-not $?) { return }
        Write-Host -fo Cyan '    Duplicate binary...'
        Copy-Item "$DependsPath\bin\wavpackdll.dll" "$DependsPath\bin\wavpack.dll"
    }
    ...
    function BuildOpus {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            tar.exe -xf "$DownloadPath\opus-$OPUS_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
    ...
    function BuildOpusfile {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            tar.exe -xf "$DownloadPath\opusfile-$OPUSFILE_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Patch build configuration...'
        Get-Content "$DownloadPath\opusfile-cmake.patch" -ErrorAction Stop | patch.exe -p1 -N -d $LocalBuildPath
        if ($Error.Count -gt 0 -or $LASTEXITCODE -gt 1) { return }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
            $null = New-Item "$LocalBuildPath\build" -ItemType Directory -Force
        ...
        Write-Host -fo Cyan '    CMake configure...'
        cmake.exe --log-level="DEBUG" -S $LocalBuildPath -B "$LocalBuildPath\build" -G 'NMake Makefiles' -DCMAKE_BUILD_TYPE="$BuildTypeCMake" -DCMAKE_INSTALL_PREFIX="$DependsPathForward" -DPKG_CONFIG_EXECUTABLE="$DependsPathForward/bin/pkgconf.exe" -DBUILD_SHARED_LIBS=ON -DCMAKE_POLICY_VERSION_MINIMUM="3.5"
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    CMake build...'
        cmake.exe --build "$LocalBuildPath\build"
        if ($LASTEXITCODE -ne 0) { return }
    ...
    function BuildSpeex {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            tar.exe -xf "$DownloadPath\speex-Speex-$SPEEX_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Patch build configuration...'
        Get-Content "$DownloadPath\speex-cmake.patch" -ErrorAction Stop | patch.exe -p1 -N -d $LocalBuildPath
        if ($Error.Count -gt 0 -or $LASTEXITCODE -gt 1) { return }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
            $null = New-Item "$LocalBuildPath\build" -ItemType Directory -Force
    ...
    function BuildLibmpg123 {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            tar.exe -xf "$DownloadPath\mpg123-$MPG123_VERSION.tar.bz2" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        ...
        Write-Host -fo Cyan '    CMake configure...'
        cmake.exe --log-level="DEBUG" -S "$LocalBuildPath\ports\cmake" -B "$LocalBuildPath\build_cmake" -G 'NMake Makefiles' -DCMAKE_BUILD_TYPE="$BuildTypeCMake" -DCMAKE_INSTALL_PREFIX="$DependsPathForward" -DPKG_CONFIG_EXECUTABLE="$DependsPathForward/bin/pkgconf.exe" -DYASM_ASSEMBLER="$DependsPathForward/bin/vsyasm.exe" -DBUILD_SHARED_LIBS=ON -DBUILD_PROGRAMS=OFF -DBUILD_LIBOUT123=OFF
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    CMake build...'
        cmake.exe --build "$LocalBuildPath\build_cmake"
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    CMake install...'
        cmake.exe --install "$LocalBuildPath\build_cmake"
    }
    ...
    function BuildLame {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            tar.exe -xf "$DownloadPath\lame-$LAME_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Patch build configuration...'
        ...
        Push-Location $LocalBuildPath
        Write-Host -fo Cyan '    NMake build...'
        nmake.exe /F "$LocalBuildPath\Makefile.MSVC" MSVCVER=Win64 MAKEDIR="$LocalBuildPath" libmp3lame.dll
        if ($LASTEXITCODE -ne 0) { Pop-Location; return }
        Pop-Location
        Write-Host -fo Cyan '    Copy libraries...'
...
"@
    ...
    function BuildTwolame {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            tar.exe -xf "$DownloadPath\twolame-$TWOLAME_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Patch build configuration...'
        Get-Content "$DownloadPath\twolame.patch" -ErrorAction Stop | patch.exe -p1 -N -d $LocalBuildPath
        if ($Error.Count -gt 0 -or $LASTEXITCODE -gt 1) { return }
        ...
        Write-Host -fo Cyan '    MSBuild build...'
        msbuild.exe 'libtwolame_dll.sln' -p:Configuration="$BuildType" -p:Platform="$BuildArch32Alt" -p:SkipUWP=true
        Pop-Location
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    Copy headers...'
...
"@
    ...
    function BuildFftw3 {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract archive...'
            7z.exe x -aoa "$DownloadPath\fftw-$FFTW_VERSION-$BuildArch-$BuildType.zip" -o"$LocalBuildPath"
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Generate library...'
        lib.exe /MACHINE:"$BuildArch" /DEF:"$LocalBuildPath\libfftw3-3.def" /OUT:"$LocalBuildPath\libfftw3-3.lib"
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    Copy headers...'
...
"@
    ...
    function BuildMusepack {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            tar.exe -xf "$DownloadPath\musepack_src_r$MUSEPACK_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Patch build configuration...'
        Get-Content "$DownloadPath\musepack-fixes.patch" -ErrorAction Stop | patch.exe -p1 -N -d $LocalBuildPath
        if ($Error.Count -gt 0 -or $LASTEXITCODE -gt 1) { return }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
            $null = New-Item "$LocalBuildPath\build" -ItemType Directory -Force
        ...
        Write-Host -fo Cyan '    CMake configure...'
        cmake.exe --log-level="DEBUG" -S $LocalBuildPath -B "$LocalBuildPath\build" -G 'NMake Makefiles' -DCMAKE_BUILD_TYPE="Debug" -DCMAKE_INSTALL_PREFIX="$DependsPathForward" -DPKG_CONFIG_EXECUTABLE="$DependsPathForward/bin/pkgconf.exe" -DBUILD_SHARED_LIBS=ON -DSHARED=ON -DCMAKE_POLICY_VERSION_MINIMUM="3.5" -DCMAKE_POLICY_DEFAULT_CMP0115="OLD"
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    CMake build...'
        cmake.exe --build "$LocalBuildPath\build"
        if ($LASTEXITCODE -ne 0) { return }
...
"@
    ...
    function BuildLibopenmpt {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract archive...'
            7z.exe x -aoa "$DownloadPath\libopenmpt-${LIBOPENMPT_VERSION}+release.msvc.zip" -o"$LocalBuildPath"
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Patch build configuration...'
        Get-Content "$DownloadPath\libopenmpt-cmake.patch" -ErrorAction Stop | patch.exe -p1 -N -d $LocalBuildPath
        if ($Error.Count -gt 0 -or $LASTEXITCODE -gt 1) { return }
        if (-not (Test-Path "$LocalBuildPath\build_cmake" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
            $null = New-Item "$LocalBuildPath\build_cmake" -ItemType Directory -Force
        ...
        Write-Host -fo Cyan '    CMake configure...'
        cmake.exe --log-level="DEBUG" -S $LocalBuildPath -B "$LocalBuildPath\build_cmake" -G 'NMake Makefiles' -DCMAKE_BUILD_TYPE="$BuildTypeCMake" -DCMAKE_INSTALL_PREFIX="$DependsPathForward" -DPKG_CONFIG_EXECUTABLE="$DependsPathForward/bin/pkgconf.exe" -DBUILD_SHARED_LIBS=ON
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    CMake build...'
        cmake.exe --build "$LocalBuildPath\build_cmake"
        if ($LASTEXITCODE -ne 0) { return }
    ...
    function BuildLibgme {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract archive...'
            tar.exe -xf "$DownloadPath\libgme-$LIBGME_VERSION-src.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Patch build configuration...'
        Get-Content "$DownloadPath\libgme-pkgconf.patch" -ErrorAction Stop | patch.exe -p1 -N -d $LocalBuildPath
        if ($Error.Count -gt 0 -or $LASTEXITCODE -gt 1) { return }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
            $null = New-Item "$LocalBuildPath\build" -ItemType Directory -Force
    ...
    function BuildFdkaac {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract archive...'
            tar.exe -xf "$DownloadPath\fdk-aac-$FDK_AAC_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
        ...
        Write-Host -fo Cyan '    CMake configure...'
        cmake.exe --log-level="DEBUG" -S $LocalBuildPath -B "$LocalBuildPath\build" -G 'NMake Makefiles' -DCMAKE_BUILD_TYPE="$BuildTypeCMake" -DCMAKE_INSTALL_PREFIX="$DependsPathForward" -DPKG_CONFIG_EXECUTABLE="$DependsPathForward/bin/pkgconf.exe" -DBUILD_SHARED_LIBS=ON -DBUILD_PROGRAMS=OFF
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    CMake build...'
        cmake.exe --build "$LocalBuildPath\build"
        if ($LASTEXITCODE -ne 0) { return }
    ...
    function BuildFaad2 {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract archive...'
            tar.exe -xf "$DownloadPath\faad2-$FAAD2_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
    ...
    function BuildFaac {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract source...'
            tar.exe -xf "$DownloadPath\faac-$FAAC_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Patch build configuration...'
        Get-Content "$DownloadPath\faac-msvc.patch" -ErrorAction Stop | patch.exe -p1 -N -d $LocalBuildPath
        if ($Error.Count -gt 0 -or $LASTEXITCODE -gt 1) { return }
        ...
        Write-Host -fo Cyan '    MSBuild build...'
        msbuild.exe 'faac.sln' -p:Configuration="$BuildType" -p:Platform="$BuildArch32Alt" -p:SkipUWP=true
        Pop-Location
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    Copy headers...'
        Copy-Item "$LocalBuildPath\include\*.h" "$DependsPath\include\" -Force
...
"@
    ...
    function BuildUtfcpp {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract archive...'
            tar.exe -xf "$DownloadPath\utfcpp-$UTFCPP_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
    ...
    function BuildTaglib {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract archive...'
            tar.exe -xf "$DownloadPath\taglib-$TAGLIB_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
    ...
    function BuildLibbs2b {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract archive...'
            tar.exe -xf "$DownloadPath\libbs2b-$LIBBS2B_VERSION.tar.bz2" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Patch build configuration...'
        Get-Content "$DownloadPath\libbs2b-msvc.patch" -ErrorAction Stop | patch.exe -p1 -N -d $LocalBuildPath
        if ($Error.Count -gt 0 -or $LASTEXITCODE -gt 1) { return }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
            $null = New-Item "$LocalBuildPath\build" -ItemType Directory -Force
    ...
    function BuildLibebur128 {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract archive...'
            tar.exe -xf "$DownloadPath\libebur128-$LIBEBUR128_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Patch build configuration...'
    ...
    function BuildFfmpeg {
        ...
        Write-Host -fo Cyan "    Checkout version $FFMPEG_VERSION..."
        git.exe -C "$LocalBuildPath" checkout meson-$FFMPEG_VERSION
        if ($LASTEXITCODE -ne 0) { return }
        git.exe -C "$LocalBuildPath" pull --rebase --autostash
        if ($LASTEXITCODE -ne 0) { return }
        if (-not (Test-Path "$LocalBuildPath\build\build.ninja")) {
            Write-Host -fo Cyan '    Meson configure...'
            meson.exe setup --buildtype="$BuildTypeMeson" --default-library=both --prefix="$DependsPath" --pkg-config-path="$DependsPath\lib\pkgconfig" --wrap-mode=nodownload -Dtests=disabled -Dgpl=enabled "$LocalBuildPath\build" $LocalBuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Ninja build...'
        ninja.exe -C "$LocalBuildPath\build"
    ...
    function BuildChromaprint {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract archive...'
            tar.exe -xf "$DownloadPath\chromaprint-$CHROMAPRINT_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
        ...
        Write-Host -fo Cyan '    CMake configure...'
        cmake.exe --log-level="DEBUG" -S $LocalBuildPath -B "$LocalBuildPath\build" -G 'NMake Makefiles' -DCMAKE_BUILD_TYPE="$BuildTypeCMake" -DCMAKE_INSTALL_PREFIX="$DependsPathForward" -DPKG_CONFIG_EXECUTABLE="$DependsPathForward/bin/pkgconf.exe" -DBUILD_SHARED_LIBS=ON -DFFMPEG_ROOT="$DependsPath" -DCMAKE_POLICY_VERSION_MINIMUM="3.5"
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    CMake build...'
        cmake.exe --build "$LocalBuildPath\build"
        if ($LASTEXITCODE -ne 0) { return }
    ...
    function BuildGstreamer {
        if ($GStreamerDevMode) {
            ...
            if (-not (Test-Path $LocalBuildPath -PathType Container)) {
                Write-Host -fo Cyan '    Extract source...'
                7z.exe x -aos "$DownloadPath\gstreamer-$GSTREAMER_VERSION.tar.xz" -o"$DownloadPath"
                if ($LASTEXITCODE -ne 0) { return }
                7z.exe x -aoa "$DownloadPath\gstreamer-$GSTREAMER_VERSION.tar" -o"$BuildPath"
                if ($LASTEXITCODE -ne 0) { return }
            }
        }
        if (-not (Test-Path "$LocalBuildPath\build\build.ninja")) {
            Write-Host -fo Cyan '    Meson configure...'
            meson.exe setup --buildtype="$BuildTypeMeson" --default-library=shared --prefix="$DependsPath" --pkg-config-path="$DependsPath\lib\pkgconfig" --wrap-mode=nodownload -Dc_args="-I$DependsPathForward/include" -Dexamples=disabled -Dtests=disabled -Dbenchmarks=disabled -Dtools=enabled -Dintrospection=disabled -Dnls=disabled -Ddoc=disabled -Dgst_debug=true -Dgst_parse=true -Dregistry=true "$LocalBuildPath\build" $LocalBuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Ninja build...'
        ninja.exe -C "$LocalBuildPath\build"
    ...
    function BuildGstpluginsbase {
        if ($GStreamerDevMode) {
            ...
            if (-not (Test-Path $LocalBuildPath -PathType Container)) {
                Write-Host -fo Cyan '    Extract source...'
                7z.exe x -aos "$DownloadPath\gst-plugins-base-$GSTREAMER_VERSION.tar.xz" -o"$DownloadPath"
                if ($LASTEXITCODE -ne 0) { return }
                7z.exe x -aoa "$DownloadPath\gst-plugins-base-$GSTREAMER_VERSION.tar" -o"$BuildPath"
                if ($LASTEXITCODE -ne 0) { return }
            }
        }
        if (-not (Test-Path "$LocalBuildPath\build\build.ninja")) {
            Write-Host -fo Cyan '    Meson configure...'
            meson.exe setup --buildtype="$BuildTypeMeson" --default-library=shared --prefix="$DependsPath" --pkg-config-path="$DependsPath\lib\pkgconfig" --wrap-mode=nodownload -Dc_args="-I$DependsPathForward/include -I$DependsPathForward/include/opus" --auto-features=disabled -Dexamples=disabled -Dtests=disabled -Dtools=enabled -Dintrospection=disabled -Dnls=disabled -Dorc=enabled -Ddoc=disabled -Dadder=enabled -Dapp=enabled -Daudioconvert=enabled -Daudiomixer=enabled -Daudiorate=enabled -Daudioresample=enabled -Daudiotestsrc=enabled -Ddsd=enabled -Dencoding=enabled -Dgio=enabled -Dgio-typefinder=enabled -Dpbtypes=enabled -Dplayback=enabled -Dtcp=enabled -Dtypefind=enabled -Dvolume=enabled -Dogg=enabled -Dopus=enabled -Dvorbis=enabled "$LocalBuildPath\build" $LocalBuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Ninja build...'
        ninja.exe -C "$LocalBuildPath\build"
    ...
    function BuildGstpluginsgood {
        if ($GStreamerDevMode) {
            ...
            if (-not (Test-Path $LocalBuildPath -PathType Container)) {
                Write-Host -fo Cyan '    Extract source...'
                7z.exe x -aos "$DownloadPath\gst-plugins-good-$GSTREAMER_VERSION.tar.xz" -o"$DownloadPath"
                if ($LASTEXITCODE -ne 0) { return }
                7z.exe x -aoa "$DownloadPath\gst-plugins-good-$GSTREAMER_VERSION.tar" -o"$BuildPath"
                if ($LASTEXITCODE -ne 0) { return }
            }
        }
        if (-not (Test-Path "$LocalBuildPath\build\build.ninja")) {
            Write-Host -fo Cyan '    Meson configure...'
            meson.exe setup --buildtype="$BuildTypeMeson" --default-library=shared --prefix="$DependsPath" --pkg-config-path="$DependsPath\lib\pkgconfig" --wrap-mode=nodownload -Dc_args="-I$DependsPathForward/include" --auto-features=disabled -Dexamples=disabled -Dtests=disabled -Dnls=disabled -Dorc=enabled -Dasm=enabled -Ddoc=disabled -Dapetag=enabled -Daudiofx=enabled -Daudioparsers=enabled -Dautodetect=enabled -Dequalizer=enabled -Dicydemux=enabled -Did3demux=enabled -Disomp4=enabled -Dreplaygain=enabled -Drtp=enabled -Drtsp=enabled -Dspectrum=enabled -Dudp=enabled -Dwavenc=enabled -Dwavparse=enabled -Dxingmux=enabled -Dadaptivedemux2=enabled -Ddirectsound=enabled -Dflac=enabled -Dlame=enabled -Dmpg123=enabled -Dspeex=enabled -Dtaglib=enabled -Dtwolame=enabled -Dwaveform=enabled -Dwavpack=enabled -Dsoup=enabled -Dhls-crypto=openssl "$LocalBuildPath\build" $LocalBuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Ninja build...'
        ninja.exe -C "$LocalBuildPath\build"
    ...
    function BuildGstpluginsbad {
        if ($GStreamerDevMode) {
            ...
            if (-not (Test-Path $LocalBuildPath -PathType Container)) {
                Write-Host -fo Cyan '    Extract source...'
                7z.exe x -aos "$DownloadPath\gst-plugins-bad-$GSTREAMER_VERSION.tar.xz" -o"$DownloadPath"
                if ($LASTEXITCODE -ne 0) { return }
                7z.exe x -aoa "$DownloadPath\gst-plugins-bad-$GSTREAMER_VERSION.tar" -o"$BuildPath"
                if ($LASTEXITCODE -ne 0) { return }
            }
        }
        Write-Host -fo Cyan '    Patch build configuration...'
        Get-Content "$DownloadPath\gst-plugins-bad-meson-dependency.patch" -ErrorAction Stop | patch.exe -p1 -N -d $LocalBuildPath
        if ($Error.Count -gt 0 -or $LASTEXITCODE -gt 1) { return }
        if (-not (Test-Path "$LocalBuildPath\build\build.ninja")) {
            Write-Host -fo Cyan '    Meson configure...'
            meson.exe setup --buildtype="$BuildTypeMeson" --default-library=shared --prefix="$DependsPath" --pkg-config-path="$DependsPath\lib\pkgconfig" --wrap-mode=nodownload -Dc_args="-I$DependsPathForward/include -I$DependsPathForward/include/opus" --auto-features=disabled -Dexamples=disabled -Dtools=enabled -Dtests=disabled -Dintrospection=disabled -Dnls=disabled -Dorc=enabled -Dgpl=enabled -Daiff=enabled -Dasfmux=enabled -Did3tag=enabled -Dmpegdemux=enabled -Dmpegpsmux=enabled -Dmpegtsdemux=enabled -Dmpegtsmux=enabled -Dremovesilence=enabled -Daes=enabled -Dasio=enabled -Dbluez=enabled -Dbs2b=enabled -Dchromaprint=enabled -Ddash=enabled -Ddirectsound=enabled -Dfaac=enabled -Dfaad=enabled -Dfdkaac=enabled -Dgme=enabled -Dmusepack=enabled -Dopenmpt=enabled -Dopus=enabled -Dwasapi=enabled -Dwasapi2=enabled -Dhls=enabled "$LocalBuildPath\build" $LocalBuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Ninja build...'
        ninja.exe -C "$LocalBuildPath\build"
    ...
    function BuildGstpluginsugly {
        if ($GStreamerDevMode) {
            ...
            if (-not (Test-Path $LocalBuildPath -PathType Container)) {
                Write-Host -fo Cyan '    Extract source...'
                7z.exe x -aos "$DownloadPath\gst-plugins-ugly-$GSTREAMER_VERSION.tar.xz" -o"$DownloadPath"
                if ($LASTEXITCODE -ne 0) { return }
                7z.exe x -aoa "$DownloadPath\gst-plugins-ugly-$GSTREAMER_VERSION.tar" -o"$BuildPath"
                if ($LASTEXITCODE -ne 0) { return }
            }
        }
        if (-not (Test-Path "$LocalBuildPath\build\build.ninja")) {
            Write-Host -fo Cyan '    Meson configure...'
            meson.exe setup --buildtype="$BuildTypeMeson" --default-library=shared --prefix="$DependsPath" --pkg-config-path="$DependsPath\lib\pkgconfig" --wrap-mode=nodownload -Dc_args="-I$DependsPathForward/include" --auto-features=disabled -Dnls=disabled -Dorc=enabled -Dtests=disabled -Ddoc=disabled -Dgpl=enabled -Dasfdemux=enabled "$LocalBuildPath\build" $LocalBuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Ninja build...'
        ninja.exe -C "$LocalBuildPath\build"
    ...
    function BuildGstlibav {
        if ($GStreamerDevMode) {
            ...
            if (-not (Test-Path $LocalBuildPath -PathType Container)) {
                Write-Host -fo Cyan '    Extract source...'
                7z.exe x -aos "$DownloadPath\gst-libav-$GSTREAMER_VERSION.tar.xz" -o"$DownloadPath"
                if ($LASTEXITCODE -ne 0) { return }
                7z.exe x -aoa "$DownloadPath\gst-libav-$GSTREAMER_VERSION.tar" -o"$BuildPath"
                if ($LASTEXITCODE -ne 0) { return }
            }
        }
        if (-not (Test-Path "$LocalBuildPath\build\build.ninja")) {
            Write-Host -fo Cyan '    Meson configure...'
            meson.exe setup --buildtype="$BuildTypeMeson" --default-library=shared --prefix="$DependsPath" --pkg-config-path="$DependsPath\lib\pkgconfig" --wrap-mode=nodownload -Dc_args="-I$DependsPathForward/include" -Dtests=disabled -Ddoc=disabled "$LocalBuildPath\build" $LocalBuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Ninja build...'
        ninja.exe -C "$LocalBuildPath\build"
    ...
    function BuildGstspotify {
        ...
        if (-not (Test-Path "$LocalBuildPath\build\build.ninja")) {
            Write-Host -fo Cyan '    Meson configure...'
            meson.exe setup --buildtype="$BuildTypeMeson" --default-library=shared --prefix="$DependsPath" --pkg-config-path="" -Dc_link_args="-L$DependsPath\lib" --wrap-mode=nodownload --auto-features=disabled -Dexamples=disabled -Dtests=disabled -Dspotify=enabled "$LocalBuildPath\build" $LocalBuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Ninja build...'
        ninja.exe -C "$LocalBuildPath\build"
    ...
    function BuildAbseil {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract archive...'
            tar.exe -xf "$DownloadPath\abseil-cpp-$ABSEIL_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
        ...
        Write-Host -fo Cyan '    CMake configure...'
        cmake.exe --log-level="DEBUG" -S $LocalBuildPath -B "$LocalBuildPath\build" -G 'NMake Makefiles' -DCMAKE_BUILD_TYPE="$BuildTypeCMake" -DCMAKE_INSTALL_PREFIX="$DependsPathForward" -DPKG_CONFIG_EXECUTABLE="$DependsPathForward/bin/pkgconf.exe" -DBUILD_SHARED_LIBS=ON -DBUILD_TESTING=OFF -DABSL_BUILD_TESTING=OFF
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    CMake build...'
        cmake.exe --build "$LocalBuildPath\build"
        if ($LASTEXITCODE -ne 0) { return }
    ...
    function BuildProtobuf {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract archive...'
            tar.exe -xf "$DownloadPath\protobuf-$PROTOBUF_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
        ...
        Write-Host -fo Cyan '    CMake configure...'
        cmake.exe --log-level="DEBUG" -S $LocalBuildPath -B "$LocalBuildPath\build" -G 'NMake Makefiles' -DCMAKE_BUILD_TYPE="$BuildTypeCMake" -DCMAKE_INSTALL_PREFIX="$DependsPathForward" -DPKG_CONFIG_EXECUTABLE="$DependsPathForward/bin/pkgconf.exe" -Dprotobuf_ABSL_PROVIDER="package" -DBUILD_SHARED_LIBS=ON -Dprotobuf_BUILD_SHARED_LIBS=ON -Dprotobuf_BUILD_TESTS=OFF -Dprotobuf_BUILD_EXAMPLES=OFF -Dprotobuf_BUILD_LIBPROTOC=OFF -Dprotobuf_BUILD_PROTOC_BINARIES=ON -Dprotobuf_WITH_ZLIB=ON
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    CMake build...'
        cmake.exe --build "$LocalBuildPath\build"
        if ($LASTEXITCODE -ne 0) { return }
    ...
    function BuildQttools {
        if ($QTDevMode) {
            ...
            if (-not (Test-Path $LocalBuildPath -PathType Container)) {
                Write-Host -fo Cyan '    Extract source...'
                7z.exe x -aos "$DownloadPath\qttools-everywhere-src-$QT_VERSION.tar.xz" -o"$DownloadPath"
                if ($LASTEXITCODE -ne 0) { return }
                7z.exe x -aoa "$DownloadPath\qttools-everywhere-src-$QT_VERSION.tar" -o"$BuildPath"
                if ($LASTEXITCODE -ne 0) { return }
            }
        }
        Write-Host -fo Cyan '    CMake configure...'
        cmake.exe --log-level="DEBUG" -S $LocalBuildPath -B "$LocalBuildPath\build" -G Ninja -DCMAKE_BUILD_TYPE="$BuildTypeCMake" -DCMAKE_INSTALL_PREFIX="$DependsPathForward" -DPKG_CONFIG_EXECUTABLE="$DependsPathForward/bin/pkgconf.exe" -DCMAKE_PREFIX_PATH="$DependsPathForward/lib/cmake" -DBUILD_SHARED_LIBS=ON -DBUILD_STATIC_LIBS=OFF -DQT_BUILD_EXAMPLES=OFF -DQT_BUILD_EXAMPLES_BY_DEFAULT=OFF -DQT_BUILD_TOOLS_WHEN_CROSSCOMPILING=ON -DFEATURE_assistant=OFF -DFEATURE_designer=OFF -DFEATURE_distancefieldgenerator=OFF -DFEATURE_kmap2qmap=OFF -DFEATURE_pixeltool=OFF -DFEATURE_qdbus=OFF -DFEATURE_qev=OFF -DFEATURE_qtattributionsscanner=OFF -DFEATURE_qtdiag=OFF -DFEATURE_qtplugininfo=OFF -DFEATURE_linguist=ON
        Write-Host -fo Cyan '    CMake build...'
        cmake.exe --build "$LocalBuildPath\build"
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    CMake install...'
    ...
    function BuildQtgrpc {
        if ($QTDevMode) {
            ...
            if (-not (Test-Path $LocalBuildPath -PathType Container)) {
                Write-Host -fo Cyan '    Extract source...'
                7z.exe x -aos "$DownloadPath\qtgrpc-everywhere-src-$QT_VERSION.tar.xz" -o"$DownloadPath"
                if ($LASTEXITCODE -ne 0) { return }
                7z.exe x -aoa "$DownloadPath\qtgrpc-everywhere-src-$QT_VERSION.tar" -o"$BuildPath"
                if ($LASTEXITCODE -ne 0) { return }
            }
        }
        Write-Host -fo Cyan '    CMake configure...'
    ...
    function BuildQtsparkle {
        ...
        Write-Host -fo Cyan '    CMake configure...'
        cmake.exe --log-level="DEBUG" -S $LocalBuildPath -B "$LocalBuildPath\build" -G 'NMake Makefiles' -DCMAKE_BUILD_TYPE="$BuildTypeCMake" -DCMAKE_INSTALL_PREFIX="$DependsPathForward" -DPKG_CONFIG_EXECUTABLE="$DependsPathForward/bin/pkgconf.exe" -DCMAKE_PREFIX_PATH="$DependsPathForward/lib/cmake" -DBUILD_WITH_QT6=ON -DBUILD_SHARED_LIBS=ON
        Write-Host -fo Cyan '    CMake build...'
        cmake.exe --build "$LocalBuildPath\build"
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    CMake install...'
    ...
    function BuildSparsehash {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract archive...'
            tar.exe -xf "$DownloadPath\sparsehash-$SPARSEHASH_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Patch build configuration...'
        Get-Content "$DownloadPath\sparsehash-msvc.patch" -ErrorAction Stop | patch.exe -p1 -N -d $LocalBuildPath
        if ($Error.Count -gt 0 -or $LASTEXITCODE -gt 1) { return }
...
"@
    ...
    function BuildRapidjson {
        ...
        Write-Host -fo Cyan '    CMake configure...'
        cmake.exe --log-level="DEBUG" -S $LocalBuildPath -B "$LocalBuildPath\build" -G 'NMake Makefiles' -DCMAKE_BUILD_TYPE="$BuildTypeCMake" -DCMAKE_INSTALL_PREFIX="$DependsPathForward" -DCMAKE_INSTALL_DIR="$DependsPath\lib\cmake\RapidJSON" -DBUILD_SHARED_LIBS=ON -DCMAKE_POLICY_VERSION_MINIMUM="3.5" -DBUILD_TESTING=OFF -DRAPIDJSON_BUILD_TESTS=OFF -DRAPIDJSON_BUILD_EXAMPLES=OFF -DRAPIDJSON_BUILD_DOC=OFF
        Write-Host -fo Cyan '    CMake build...'
        cmake.exe --build "$LocalBuildPath\build"
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    CMake install...'
    ...
    function BuildKdsingleapp {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract archive...'
            tar.exe -xf "$DownloadPath\kdsingleapplication-$KDSINGLEAPPLICATION_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
        ...
        Write-Host -fo Cyan '    CMake configure...'
        cmake.exe --log-level="DEBUG" -S $LocalBuildPath -B "$LocalBuildPath\build" -G 'NMake Makefiles' -DCMAKE_BUILD_TYPE="$BuildTypeCMake" -DCMAKE_INSTALL_PREFIX="$DependsPathForward" -DPKG_CONFIG_EXECUTABLE="$DependsPathForward/bin/pkgconf.exe" -DCMAKE_PREFIX_PATH="$DependsPathForward/lib/cmake" -DBUILD_SHARED_LIBS=ON -DKDSingleApplication_QT6=ON
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    CMake build...'
        cmake.exe --build "$LocalBuildPath\build"
        if ($LASTEXITCODE -ne 0) { return }
    ...
    function BuildGlew {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract archive...'
            tar.exe -xf "$DownloadPath\glew-$GLEW_VERSION.tgz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Patch build configuration...'
        (Get-Content "$LocalBuildPath\build\cmake\CMakeLists.txt" -Raw) -replace '(?m)^(\s*)(target_link_libraries\s*\(\s*glew\s*LINK_PRIVATE\s*-BASE:0x[0-9a-f]+\s*\)\s*)(\r?\n)',
        '$1$2$3$1target_link_libraries (glew LINK_PRIVATE libvcruntime.lib)$3$1target_link_libraries (glew LINK_PRIVATE msvcrt.lib)$3' | Set-Content "$LocalBuildPath\build\cmake\CMakeLists.txt" -NoNewline
        if (-not $?) { return }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
            $null = New-Item "$LocalBuildPath\build" -ItemType Directory -Force
        ...
        Write-Host -fo Cyan '    CMake configure...'
        cmake.exe --log-level="DEBUG" -S "$LocalBuildPath\build\cmake" -B "$LocalBuildPath\build" -G 'NMake Makefiles' -DCMAKE_BUILD_TYPE="$BuildTypeCMake" -DCMAKE_INSTALL_PREFIX="$DependsPathForward" -DPKG_CONFIG_EXECUTABLE="$DependsPathForward/bin/pkgconf.exe" -DBUILD_SHARED_LIBS=ON -DCMAKE_POLICY_VERSION_MINIMUM="3.5"
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    CMake build...'
        cmake.exe --build "$LocalBuildPath\build"
        if ($LASTEXITCODE -ne 0) { return }
    ...
    function BuildLibprojectm {
        ...
        if (-not (Test-Path $LocalBuildPath -PathType Container)) {
            Write-Host -fo Cyan '    Extract archive...'
            tar.exe -xf "$DownloadPath\libprojectm-$LIBPROJECTM_VERSION.tar.gz" -C $BuildPath
            if ($LASTEXITCODE -ne 0) { return }
        }
        if (-not (Test-Path "$LocalBuildPath\build" -PathType Container)) {
            Write-Host -fo Cyan '    Create build directory...'
    ...
    function BuildGettext {
        ...
        if (-not (Test-Path "$BuildPath\gettext${GETTEXT_VERSION}-iconv*" -PathType Container)) {
            ...
            Write-Host -fo Cyan '    Extract archive...'
            7z.exe x -aoa $SourceZip -o"$LocalBuildPath"
            if ($LASTEXITCODE -ne 0) { return }
        }
        Write-Host -fo Cyan '    Copy binaries...'
...
"@
    ...
    function BuildStrawberry {
        ...
        if ($BuildArch -eq 'x86') {
            ...
            (Get-Content "$LocalBuildPath\src\engine\gstfastspectrum.cpp" -Raw) -replace 'size -= block_size \* bpf;', 'size -= static_cast<gsize>(block_size * bpf);' | Set-Content "$LocalBuildPath\src\engine\gstfastspectrum.cpp" -NoNewline
            if (-not $?) { return }
        }
        $StrawbArch = $BuildArch -creplace 'x64', 'x86_64'
        ...
        Write-Host -fo Cyan '    CMake configure...'
        cmake.exe --log-level="DEBUG" -S $LocalBuildPath -B "$LocalBuildPath\build" -G 'NMake Makefiles' -DCMAKE_BUILD_TYPE="$BuildTypeCMake" -DCMAKE_INSTALL_PREFIX="$DependsPathForward" -DCMAKE_PREFIX_PATH="$DependsPathForward" -DICU_ROOT="$DependsPath" -DARCH="$StrawbArch" -DENABLE_SPOTIFY="$IncludeSpotify" -DENABLE_TRANSLATIONS=ON -DBUILD_WERROR=ON -DENABLE_WIN32_CONSOLE="$EnableCon" -DENABLE_AUDIOCD=OFF -DENABLE_MTP=OFF -DENABLE_GPOD=OFF
        if ($LASTEXITCODE -ne 0) { return }
        Write-Host -fo Cyan '    CMake build...'
        cmake.exe --build "$LocalBuildPath\build"
        if ($LASTEXITCODE -ne 0) { return }
        ...
        makensis.exe "$LocalBuildPath\build\strawberry.nsi"
    }
    ...
    $BUILD_TARGETS = [ordered]@{
        ...
        "Qttools $QTToolsVer"                           = 'BuildQttools', '*', "$DependsPath\bin\linguist.exe"
    ...
    :TargetLoop foreach ($Target in $BUILD_TARGETS.Keys) {
        ...
        if ($NeedsBuild) {
            ...
            if (-not $WasForced -and $ArchFilter -cne '*' -and $BuildArch -notmatch $ArchFilter) {
                Write-Host -fo White ('{0,-70}' -f "${Target}...  ") -NoNewline
                Write-Host -fo Yellow 'SKIPPED (ARCH)'
                continue TargetLoop
            }
            $Title = " $Target "
            ...
            if (-not ($Success -and $Exists)) {
                $Err = [ErrorRecord]::new([OperationCanceledException]::new("Failed to build '$Target' after $MaxBuildTries attempts."), 'BuildFailed', 'OperationStopped', $Target)
                $PSCmdlet.WriteError($Err)
    ...
    if (-not $NoBuild) {
        ...
        if ($OverallSuccess -and -not $NoStrawberry) {
            $InstPath = Get-Item "$BuildPath\_strawberry\build\strawberrysetup*.exe" | Select-Object -ExpandProperty FullName
